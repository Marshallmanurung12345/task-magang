...
<div class="row">
    ...
    <div class="col-md-9">
        <div class="card border-0 shadow-sm rounded-4 p-4">
            <h2 class="mb-1" style="font-family: 'Outfit', sans-serif;">Edit Berita</h2>
            <p class="text-muted mb-4 small">Perbarui data berita yang sudah diterbitkan.</p>
            ...
            <form action="{{ route('news.update', $news->slug) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" class="form-control rounded-3 @error('title') is-invalid @enderror" id="title" name="title" value="{{ old('title', $news->title) }}" placeholder="Masukkan judul berita" required>
                    @error('title')
                ...
                <div class="mb-4">
                    ...
                    @endif
                    <input type="file" class="form-control rounded-3 @error('thumbnail') is-invalid @enderror" id="thumbnail" name="thumbnail" accept="image/jpeg,image/png,image/jpg,image/webp">
                    <div class="form-text">Biarkan kosong jika tidak ingin mengubah thumbnail. Format: JPG, JPEG, PNG, WEBP. Maksimal 2 MB.</div>
                    @error('thumbnail')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea class="form-control @error('content') is-invalid @enderror" id="editor" name="content" rows="10" placeholder="Tulis isi berita di sini...">{{ old('content', $news->content) }}</textarea>
                    @error('content')
